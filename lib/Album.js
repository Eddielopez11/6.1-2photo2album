...
export default React.createClass({
  ...
  render() {
    ...
    let currentImageAlbum = this.props.album[this.props.params.imgOne]
    return (
      <div className="album__div">
        <h1>album</h1>
        {
          currentImageAlbum.map((image) => {
            return <div>
                      <img className={ this.state.clicked === true? "image image--clicked" : "image"} onClick={this.onClickImg}
                           src={image.imgOne}
                           alt="img one"/>
                         <img className={ this.state.clicked === true? "image image--clicked" : "image"} onClick={this.onClickImg}
                           src={image.imgTwo}
